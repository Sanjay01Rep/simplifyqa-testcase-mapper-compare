 ...
 * Loads project .env and provides token management for SimplifyQA API authentication.
 */
const fs = require("fs");
...
function loadProjectEnv(rootDir, override = true) {
...
const POSSIBLE_TOKEN_KEYS = [
  "SIMPLIFYQA_BEARER_TOKEN",
  "SIMPLIFY_QA_BEARER_TOKEN",
  "SIMPLIFYQA_TOKEN",
  "SIMPLIFY_QA_TOKEN",
  "BEARER_TOKEN",
  "AUTH_TOKEN",
...
function getBearerToken(rootDir) {
  ...
  if (fs.existsSync(envPath)) {
    try {
      ...
      for (const raw of text.split(/\r?\n/)) {
        ...
        if (POSSIBLE_TOKEN_KEYS.includes(key)) {
  ...
  for (const k of POSSIBLE_TOKEN_KEYS) {
    if (process.env[k]) {
      return normalizeBearerToken(process.env[k]);
    }
  }
...
function getTokenStatus(rootDir) {
  ...
  if (placeholder) {
    return {
      ...
      expiresAt: null,
      expiresInMs: null,
      message: "No token set in .env. Click Change token to set one.",
    };
  }
  ...
  if (expiresAtMs == null) {
    return {
      ...
      expiresAt: null,
      expiresInMs: null,
      message: "Token loaded from .env · Ready for SimplifyQA Live export.",
    };
  }
  ...
  return {
    ...
    expiresInMs,
    message: expired
      ? "Token in .env has expired. Click Change token to update."
      : `Token loaded from .env · OK (expires in ~${formatDuration(expiresInMs)}).`,
  };
}
