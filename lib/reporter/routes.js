...
function nextCustomTemplateIndex(props) {
  for (let i = 5; i <= 50; i++) {
    if (!props[`TEMPLATE_${i}`] && !props[`OUTPUT_FILE_${i}`]) return i;
  }
  return 51;
}

const BUILTIN_TEMPLATE_BASENAMES = new Set([
  "fms status tracker.xlsx",
  "fms 4 ep template.xlsx",
  "e2e report.xlsx",
  "sit template.xlsx",
]);

function canonicalTemplateKey(fileName) {
  return path
    .parse(path.basename(fileName || ""))
    .name.replace(/\s*\(\d+\)$/, "")
    .trim()
    .toLowerCase();
}

function isBuiltInTemplateName(fileName) {
  return BUILTIN_TEMPLATE_BASENAMES.has(path.basename(fileName || "").toLowerCase());
}

function findCustomSlotForUpload(props, requestedName) {
  const wantRel = `Template/${path.basename(requestedName)}`.replace(/\\/g, "/");
  const wantKey = canonicalTemplateKey(requestedName);
  let sameStem = null;
  for (let i = 5; i <= 50; i++) {
    const rel = String(props[`TEMPLATE_${i}`] || "").replace(/\\/g, "/");
    if (!rel) continue;
    if (rel === wantRel) return i;
    if (canonicalTemplateKey(rel) === wantKey) sameStem = sameStem == null ? i : sameStem;
  }
  return sameStem;
}

function uniqueTemplateFileName(templateDir, originalName) {
  const base = path.basename(originalName);
  const parsed = path.parse(base);
  let name = `${parsed.name}${parsed.ext}`;
  let n = 2;
  while (fs.existsSync(path.join(templateDir, name))) {
...
function unregisterTemplateInProperties(propsPath, index) {
  let text = fs.existsSync(propsPath) ? fs.readFileSync(propsPath, "utf8") : "";
  const tKey = `TEMPLATE_${index}`;
  const oKey = `OUTPUT_FILE_${index}`;
  text = text.replace(new RegExp(`^${tKey}=.*(?:\\r?\\n)?`, "m"), "");
  text = text.replace(new RegExp(`^${oKey}=.*(?:\\r?\\n)?`, "m"), "");
  if (new RegExp(`^TEMPLATE_CHOICE=${index}\\s*$`, "m").test(text)) {
    text = text.replace(/^TEMPLATE_CHOICE=.*$/m, "TEMPLATE_CHOICE=1");
  }
  text = text.replace(/\n{3,}/g, "\n\n");
...
function createReporterRouter() {
  ...
  router.post("/upload-template", templateUpload.single("template"), async (req, res) => {
    try {
      ...
      const props = loadProperties(PROPERTIES_PATH);
      const requestedName = path.basename(file.originalname);
      let targetIndex = findCustomSlotForUpload(props, requestedName);
      let safeFileName;
      let previousRelative = targetIndex ? String(props[`TEMPLATE_${targetIndex}`] || "") : "";

      if (isBuiltInTemplateName(requestedName) && targetIndex == null) {
        safeFileName = uniqueTemplateFileName(templateDir, requestedName);
      } else {
      ...
      if (targetIndex == null) {
        targetIndex = nextCustomTemplateIndex(props);
      }
      if (targetIndex > 50) {
        return res.status(400).json({
          ok: false,
          message: "No free template slot left (1–4 are built-in; 5–50 are for uploads).",
        });
      }

      registerTemplateInProperties(PROPERTIES_PATH, targetIndex, templateRelative, outputRelative);

      const prevBase = previousRelative ? path.basename(previousRelative) : "";
      if (
        prevBase &&
        prevBase.toLowerCase() !== safeFileName.toLowerCase() &&
        canonicalTemplateKey(prevBase) === canonicalTemplateKey(safeFileName)
      ) {
        const leftover = path.join(templateDir, prevBase);
        if (fs.existsSync(leftover) && leftover.toLowerCase() !== targetPath.toLowerCase()) {
          try {
      ...
      try {
        ...
        const moduleSections =
          ws && typeof discoverStatusSections === "function"
            ? discoverStatusSections(ws)
            : [];
        if (moduleSections.length) {
          TEMPLATE_META[targetIndex] = {
            statusSections: moduleSections.length,
            description: `${moduleSections.length} execution-plan block(s).`,
          };
        } else {
          const wsSections = workstreamFill.discoverWorkstreamStatusSections(ws);
          if (wsSections.length) {
            TEMPLATE_META[targetIndex] = {
              statusSections: wsSections.length,
              kind: "workstream",
              description:
  ...
  router.post("/delete-template", async (req, res) => {
    try {
      const choice = Number(req.body && req.body.choice);
      if (!Number.isInteger(choice) || choice < 5 || choice > 50) {
        return res.status(400).json({
          ok: false,
          message: "Only uploaded templates (5–50) can be removed from the dropdown. Built-in templates 1–4 stay.",
        });
      }
      const props = loadProperties(PROPERTIES_PATH);
      const templateRelative = String(props[`TEMPLATE_${choice}`] || "").trim();
      if (!templateRelative) {
        return res.status(404).json({
          ok: false,
          message: `Template ${choice} is not in the dropdown.`,
        });
      }

      unregisterTemplateInProperties(PROPERTIES_PATH, choice);
      delete TEMPLATE_META[choice];
      delete TEMPLATE_META[String(choice)];

      const updatedProps = loadProperties(PROPERTIES_PATH);
      const fileName = path.basename(templateRelative);
      res.json({
        ok: true,
        message: `Template ${choice} (${fileName}) removed from the dropdown.`,
        choice: String(updatedProps.TEMPLATE_CHOICE || "1"),
        form: formDefaultsFromProps(updatedProps),
      });
    } catch (err) {
      res.status(500).json({ ok: false, message: err.message || String(err) });
  ...
  router.get("/plans", async (req, res) => {
    try {
      ...
      const idsRaw = String((req.query && req.query.ids) || "").trim();
      const planIds = idsRaw
        ? idsRaw.split(/[,\s]+/).map((s) => s.trim()).filter(Boolean)
        : [];
      const token = getBearerToken(ROOT);
      const plans = await fetchSimplifyQaExecutionPlans(token, projectId, planIds);
      res.json({ ok: true, projectId, plans });
    } catch (err) {
      res.status(500).json({
        ok: false,
  ...
  router.get("/form-defaults", async (_req, res) => {
    try {
      ...
      try {
        form.plans = await fetchSimplifyQaExecutionPlans(token, form.projectId, form.planIds);
      } catch {}
  ...
  router.post("/run", async (req, res) => {
    ...
    try {
      ...
      const payload = {
        ...
        extraModulesAdded: Boolean(result.extraModulesAdded),
        preview,
        excelOpen,
        pdf: pdfInfo,
        download: {
