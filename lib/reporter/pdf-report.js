...
function passRateFillHex(value) {
  const n = Number(String(value || "").replace("%", "").trim());
  if (!Number.isFinite(n)) return null;
  if (n <= 50) return "#FF0000";
  if (n <= 80) return "#FFC000";
  return "#92D050";
}
...
function drawTable(doc, startX, startY, headers, rows, colWidths, opts = {}) {
  ...
  const passRateCol = opts.passRateCol;
  let x = startX;
  let y = startY;
  ...
  for (const row of rows) {
    ...
    row.forEach((val, i) => {
      const rateFill =
        passRateCol != null && i === passRateCol ? passRateFillHex(val) : null;
      if (rateFill) {
        doc.save();
        doc.rect(x, y, colWidths[i], rowH).fill(rateFill);
        doc.restore();
      } else if (isTotal) {
        doc.save();
