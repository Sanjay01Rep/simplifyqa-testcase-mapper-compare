...
function isTanzaniaDefectSection(section) {
  const entity = String((section && section.entity) || "").toLowerCase();
  const title = String((section && section.title) || "").toLowerCase();
  return (
    entity.includes("tanzania") ||
    title.includes("tanzania") ||
    /\btz\b/.test(entity) ||
    /\btz\b/.test(title)
  );
}
...
function entityLabelVariants(label) {
  ...
  } else if (t.includes("tanzania")) {
    ["General Tanzania", "Tanzania", "Gen TZ"].forEach((x) => out.add(x));
  }
  return [...out];
}
    ...
    if (!defectSections.length) {
      ...
      if (args.dryRun) {
        ...
        if (args.localDefects) {
          ...
          const defectProjectIds = sitRun
            ? [Number(runtime.projectId)].filter(Boolean)
            : [
                ...new Set(
                  mapped
          ...
          if (sitRun) {
            log(
              "\nSIT: Tanzania defects are not pulled. General Tanzania Defect Summary is left as in the template."
            );
          }
          const aggs = [];
          for (const pid of defectProjectIds) {
            ...
            log(`\nDownloading defect export for projectId=${pid}...`);
            ...
            aggs.push(aggregateDefectExport(defectPath));
          }
        ...
        for (const [entityKey, modMap] of defectAgg.byEntity.entries()) {
          if (sitRun && isTanzaniaDefectSection({ entity: entityKey, title: entityKey })) {
            continue;
          }
          const extras = [];
          ...
          extrasByEntity.set(String(entityKey).toLowerCase(), extras);
          extraModulesFound.push({
            source: "defects",
            entity: entityKey,
            modules: extras,
          ...
          if (args.addMissingModules) {
            log(
              `\nIncluding extra defect modules in this report (output only; template file unchanged) for [${entityKey}]:`
            );
        ...
        for (let i = 0; i < sectionCount; i++) {
          ...
          if (sitRun && isTanzaniaDefectSection(section)) {
            log(
              `\nSIT: leaving "${section.title}" as in the template (Tanzania defects were not pulled).`
            );
            continue;
          }
          if (!section.entity) {
          ...
          const extraRows = args.addMissingModules
            ? extrasForDefectSection(extrasByEntity, section.entity)
            : [];
