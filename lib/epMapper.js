...
function parseEntityFilterList(entity) {
  const parts = Array.isArray(entity) ? entity : String(entity || "").split(/[,;]/);
  return [...new Set(parts.map((v) => String(v || "").trim()).filter(Boolean))];
}

function entityPartsFromText(entityText) {
  return String(entityText || "")
    .split(/[,;/|]/)
    .map((part) => cleanSimplifyQaEntity(String(part).trim()))
    .filter(Boolean);
}

function entityTextMatchesFilter(entityText, filterEntity) {
  const filterNorm = normalizeTextMatch(filterEntity);
  if (!filterNorm) return false;
  const haystackNorm = normalizeTextMatch(entityText);
  return entityPartsFromText(entityText).some((part) => {
    const partNorm = normalizeTextMatch(part);
    return partNorm === filterNorm || partNorm.includes(filterNorm) || haystackNorm.includes(filterNorm);
  });
}

function entityTextMatchesAny(entityText, filterEntities) {
  if (!filterEntities.length) return true;
  return filterEntities.some((filterEntity) => entityTextMatchesFilter(entityText, filterEntity));
}

function matchingSelectedEntities(entityText, filterEntities) {
  if (!filterEntities.length) return [];
  return filterEntities.filter((filterEntity) => entityTextMatchesFilter(entityText, filterEntity));
}

function firstMatchingEntity(entityText, filterEntities) {
  return matchingSelectedEntities(entityText, filterEntities)[0] || "";
}

function customFieldStringsMatchAny(rawCustomFieldStrings, filterEntities) {
  if (!filterEntities.length) return true;
  const strings = Array.isArray(rawCustomFieldStrings) ? rawCustomFieldStrings : [];
  return filterEntities.some((filterEntity) => {
    const filterNorm = normalizeTextMatch(filterEntity);
    if (!filterNorm) return false;
    return strings.some((s) => {
      const cleaned = cleanSimplifyQaEntity(s);
      const parts = String(cleaned || s)
        .split(/[,;/|]/)
        .map((p) => p.trim())
        .filter(Boolean);
      return parts.some((p) => {
...
function assigneeForNamedEntity(entityName, entityAssignees, fallbackEmail) {
  const pairs = Array.isArray(entityAssignees) ? entityAssignees : [];
  const want = normalizeTextMatch(entityName);
  if (want) {
    const pair = pairs.find((p) => normalizeTextMatch(p.entity) === want);
    if (pair) return String(pair.email || "").trim();
  }
  return String(fallbackEmail || "").trim();
}

function resolveAssigneeEmail(tc, entityAssignees, fallbackEmail) {
  const pairs = Array.isArray(entityAssignees) ? entityAssignees : [];
  const named = pairs.filter((p) => String((p && p.entity) || "").trim());
  if (named.length) {
    const matched = firstMatchingEntity(tc && tc.entity, named.map((p) => p.entity));
    if (matched) {
      const pair = named.find((p) => normalizeTextMatch(p.entity) === normalizeTextMatch(matched));
      return String((pair && pair.email) || "").trim();
    }
  }
  if (pairs.length === 1) {
    return String(pairs[0].email || fallbackEmail || "").trim();
  }
  return String(fallbackEmail || "").trim();
}
...
function extractTestcasesFromSummary(bufferOrWorkbook, options = {}) {
  ...
  const filterEntities = parseEntityFilterList(options.entity);
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    // Apply entity filter if supplied (match any selected entity)
    if (filterEntities.length) {
      if (!tcEntity) continue;
      if (!entityTextMatchesAny(tcEntity, filterEntities)) {
        continue;
      }
    }
...
async function fetchTestcasesFromSimplifyQa({
  ...
  const filterEntities = parseEntityFilterList(entity);
  ...
  for (const r of records) {
    ...
    // Entity filter (match any selected entity)
    if (filterEntities.length && !customFieldStringsMatchAny(rawCustomFieldStrings, filterEntities)) {
      continue;
    }
    ...
    testcases.push({
      ...
      entity: tcEntity || (filterEntities.length === 1 ? filterEntities[0] : "") || "",
...
function buildEpWorkbook({
  ...
  const assigneePairs = Array.isArray(entityAssignees) ? entityAssignees : [];
  const fallbackAssignee = String(assigneeEmail || "").trim();
  const namedEntities = [
    ...new Set(
      (assigneePairs.map((p) => String((p && p.entity) || "").trim()).filter(Boolean).length
        ? assigneePairs.map((p) => String((p && p.entity) || "").trim()).filter(Boolean)
        : parseEntityFilterList(entityName))
    ),
  ];
  const splitByEntity = namedEntities.length > 1;

  const sheetGroups = [];
  for (const [modName, tcs] of moduleGroups.entries()) {
    if (!splitByEntity) {
      sheetGroups.push({ modName, entityName: namedEntities[0] || "", tcs });
      continue;
    }
    const byEntity = new Map();
    for (const tc of tcs) {
      const matchedEntities = matchingSelectedEntities(tc.entity, namedEntities);
      for (const matched of matchedEntities) {
        if (!byEntity.has(matched)) byEntity.set(matched, []);
        byEntity.get(matched).push(tc);
      }
    }
    for (const entity of namedEntities) {
      const list = byEntity.get(entity);
      if (!list || !list.length) continue;
      sheetGroups.push({ modName, entityName: entity, tcs: list });
    }
  }

  for (const group of sheetGroups) {
    const { modName, entityName: sheetEntity, tcs } = group;
    const rawName = splitByEntity && sheetEntity ? `${modName}_${sheetEntity}` : modName;
    const sName = getUniqueSheetName(rawName);
    ...
    for (const tc of tcs) {
      rows.push([
        ...
        finalDate,
        splitByEntity && sheetEntity
          ? assigneeForNamedEntity(sheetEntity, assigneePairs, fallbackAssignee)
          : resolveAssigneeEmail(tc, assigneePairs, fallbackAssignee),
      ]);
    }
  ...
  const assigneeSummary = assigneePairs.length
    ? assigneePairs
        .map((p) => {
          const ent = String(p.entity || "").trim();
          const email = String(p.email || "").trim();
          return ent ? `${ent} → ${email || "(none)"}` : email || "(none)";
        })
        .join("; ")
    : fallbackAssignee;
  ...
  return {
    ...
    summary: {
      ...
      modules: moduleList,
      entityName,
      entities: parseEntityFilterList(entityName),
      version: normVersion(""),
      executionType: normExecType(""),
      assignedDate: finalDate,
      assigneeEmail: fallbackAssignee,
      assigneeEmails: assigneeSummary,
      entityAssignees: assigneePairs,
    },
  };
}
