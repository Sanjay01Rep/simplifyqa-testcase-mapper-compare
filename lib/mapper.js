...
function parseClientWorkbook(source, clientFileName, config) {
  ...
  const selectedSheets = allSheetNames.length === 2 ? allSheetNames.slice(0, 2) : [allSheetNames[0]];
  if (selectedSheets.length === 2) {
    issues.push({
      type: "SHEETS_AUTO_SELECTED",
      severity: "INFO",
      message: `Workbook has 2 sheets. Auto-selected both sheets for mapping: ${selectedSheets.join(", ")}.`,
    });
  }
  const testcases = [];
  let namedTcRows = 0;
  ...
  return {
    ...
    sheetNames: selectedSheets.slice(),
    testcases,
    issues,
    namedTcRows,
    inferredTcCount,
...
function parseKenyaExcel(source, fileName) {
  ...
  for (const sheet of wb.SheetNames) {
    ...
    if (map.name == null || map.prereq == null) {
      issues.push({
        type: "KENYA_HEADER",
        severity: "WARN",
        message: `Kenya sheet "${sheet}" has no Name + Pre-Requisite columns detected.`,
      });
      continue;
    }
  ...
  if (!entries.length) {
    issues.push({
      type: "KENYA_EMPTY",
      severity: "INFO",
      message: `No filled Name + Pre-Requisite pairs in Kenya file "${fileName}". Empty pre-requisites are ignored.`,
    });
  }

  return { entries, issues, sourceType: "excel", fileName };
}
...
async function parseKenyaSource(fileName, buffer) {
  assertXlsx(fileName, "Kenya document");
  return parseKenyaExcel(buffer, fileName);
}
...
function applyKenyaPrereqs(testcases, kenya) {
  ...
  for (const tc of testcases) {
    ...
    issues.push({
      type: "PREREQ_MATCHED",
      severity: "FIX",
      message: `Pre-Requisite mapped from Kenya doc (exact name) for "${tc.name}".`,
      testcase: tc.name,
    });
  }
  ...
  for (const entry of kenya.entries) {
    ...
    if (!usedKenya.has(key)) {
      issues.push({
        ...
        severity: "INFO",
        message: `Kenya Pre-Requisite not used (no exact client testcase name match): "${entry.name}".`,
        testcase: entry.name,
      });
    }
  }
...
function buildOutputRows(testcases, config) {
  ...
  for (const tc of testcases) {
    ...
    tc.steps.forEach((step, idx) => {
      ...
      outRows.push([
        isFirst ? tc.name : "",
        isFirst ? config.Versions : "",
        isFirst ? tc.prerequisites || "" : "",
        isFirst ? tc.description : "",
        isFirst ? config.Module : "",
        "",
        isFirst ? config.TestcaseType : "",
        "",
        "",
        step.seq,
        step.stepDesc,
