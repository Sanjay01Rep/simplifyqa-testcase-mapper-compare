...
function parseForCompare(buffer, fileName, clientSheet) {
  const config = {
    Module: "Compare",
    Entity: "Compare",
    Versions: "v1.0",
  ...
  return parseClientWorkbook(buffer, fileName, config);
}
...
async function compareClientDocs({
  ...
  const parsedA = parseForCompare(bufferA, fileAName, clientSheetA);
  const parsedB = parseForCompare(bufferB, fileBName, clientSheetB);
  ...
  const includeSheetA = result.unmatchedA.length > 0;
  const includeSheetB = result.unmatchedB.length > 0;
  const rowsA = includeSheetA
    ? buildOutputRows(result.unmatchedA, { ...baseConfig, Entity: uniqueAEntity })
    : [];
  const rowsB = includeSheetB
    ? buildOutputRows(result.unmatchedB, { ...baseConfig, Entity: uniqueBEntity })
    : [];

  const used = new Set(["common"]);
  const sheetCommon = "Common";
  const sheetA = includeSheetA ? excelSheetName(fileAName, used) : "";
  const sheetB = includeSheetB ? excelSheetName(fileBName, used) : "";

  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, aoaToSheetOmitEmpty(commonRows), sheetCommon);
  if (includeSheetA) {
    XLSX.utils.book_append_sheet(wb, aoaToSheetOmitEmpty(rowsA), sheetA);
  }
  if (includeSheetB) {
    XLSX.utils.book_append_sheet(wb, aoaToSheetOmitEmpty(rowsB), sheetB);
  }
  if (!includeSheetA) {
    issues.push({
      type: "UNIQUE_SHEET_OMITTED",
      severity: "INFO",
      message: `No unique testcases in Excel A (${fileAName}) — Unique A sheet not created.`,
    });
  }
  if (!includeSheetB) {
    issues.push({
      type: "UNIQUE_SHEET_OMITTED",
      severity: "INFO",
      message: `No unique testcases in Excel B (${fileBName}) — Unique B sheet not created.`,
    });
  }
  ...
  const summary = {
    ...
    mapperHeader: String(mapperHeader || "").trim(),
    module: moduleName,
    entity: commonEntity,
...
function writeCompareLog(logPath, summary) {
  ...
  lines.push(`Mapper file  : ${summary.kenyaFile || "(none)"}`);
  lines.push(`Mapper header: ${summary.mapperHeader || "(Pre-Requisite default)"}`);
  ...
  lines.push(
    `Unmatched A (${summary.sheets.a || "omitted"}) : ${summary.counts.unmatchedA}${
      summary.counts.unmatchedA ? "" : " — Unique A sheet not created"
    }`
  );
  lines.push(
    `Unmatched B (${summary.sheets.b || "omitted"}) : ${summary.counts.unmatchedB}${
      summary.counts.unmatchedB ? "" : " — Unique B sheet not created"
    }`
  );
  lines.push(
  ...
  lines.push("");
  lines.push(`--- UNMATCHED A (${summary.sheets.a || "omitted"}) ---`);
  ...
  lines.push("");
  lines.push(`--- UNMATCHED B (${summary.sheets.b || "omitted"}) ---`);
